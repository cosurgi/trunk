...
/* ?? */	void becomeDampingTable(not_complex dampMarginMin ,not_complex dampMarginMax , not_complex dampExponent_left , not_complex dampExponent_right , bool dampFormulaSmooth , const IToX_func& iToX, const start_func& start)
		{
			for(auto size : dim_n) if((size%2)==1) std::cerr << "\nERROR: NDimTable has o̲d̲d̲ ̲s̲i̲z̲e̲,\n FFTW is best with 2ᵃ 3ᵇ 5ᶜ 7ᵈ 11ᵉ 13ᶠ, for e+f either 0 or 1\n";
			DimN pos_i(rank_d,0);
			auto dampingFunc = [](not_complex n, not_complex band_min, not_complex band_max, not_complex size , not_complex dampExponent_left , not_complex dampExponent_right , bool dampFormulaSmooth )->not_complex  {
...
// ~/Sienkiewicz/absorbtion-NDimTable-wzor.nb
				if(dampFormulaSmooth) {
					return std::exp(not_complex(
						-(
						    (n<=band_min)? dampExponent_left : dampExponent_right
						 )*std::pow(
							1.0/ (1 -	(
